---
name: scaffold
description: "Bootstrap a repository's CLAUDE.md scaffolding once `docs/CONCEPT.md` and `docs/ARCHITECTURE.md` already exist. Use this skill when the user says any of: 'scaffold this repo', 'set up CLAUDE.md', 'bootstrap this repo for Claude', 'initialize the agent scaffolding', 'create the CLAUDE.md files', 'wire up the CLAUDE.md', or any phrasing that implies a fresh repo needs its CLAUDE.md files written now that the anchoring docs are done. Also triggers on 'I've written my CONCEPT and ARCHITECTURE, now what?' or 'create root and subtree CLAUDE.md'. Writes root + per-subtree CLAUDE.md from templates in this skill's folder, establishes the per-entry `docs/changelog/` convention and the agent-owned `docs/CLAUDE_TODO.md` follow-up list, optionally wires git-describe app versioning, prints next steps. When CLAUDE.md files already exist it never overwrites blind: it audits each one against this skill's current recommendations, reports per file what's missing or outdated (e.g. an old review mandate, a changelog section still on the retired single-file numbered scheme, a required-reading block still loading a curated changelog digest), and asks which files to bring up to date — applying only surgical, additive updates that preserve project-specific content. So also trigger this skill on 'audit my CLAUDE.md', 'are my CLAUDE.md files up to date', 'check my CLAUDE.md against the latest recommendations / scaffold conventions', or 'update CLAUDE.md to the current conventions'."
---

# scaffold

Short bootstrap helper. Run on a repo that already has `docs/CONCEPT.md` and `docs/ARCHITECTURE.md`. Writes the minimal CLAUDE.md scaffolding — root + per-subtree — and nothing else. Stops there. Domain skills, memory, hooks are out of scope.
...
- **Git workflow** — ask **two** questions, not one, because these are independent axes that compose rather than a single either/or. Neither can be inferred, so always ask both.
  ...
  **Neither answer touches the Changelog discipline or Owed follow-ups sections.** The entry file is written the same way on `main`, on a branch and in a worktree, and so is a follow-up line, so nothing in either section is mode-dependent and there is nothing to keep or trim there. Don't add a conditional to either.
...
Two rubric items can't be judged from the files at all, and both are one-line disk checks: **`test -f docs/CHANGELOG-DIGEST.md`** (R1) and **`test -f docs/CLAUDE_TODO.md`** (R8). Each finding is keyed to the file on disk rather than to whether a `CLAUDE.md` references it — a digest whose reference was hand-removed is still a file people append to and nothing reads, and a `CLAUDE_TODO.md` in a repo whose `CLAUDE.md` never mentions one is that same orphan in the other convention. Reading only the `CLAUDE.md` files would never see either. Run both once, here, and pair each with what the text says: R1's migration enumerates its four combinations, and R8's disk-check bullet covers the TODO orphan — including why the items already in such a file must be handed back to the user rather than silently adopted as the agent's own.
...
One exception to narrowing: the **changelog-layout migration** spans R1, R3, whichever section carries the landing sequence (R6 for a Branch+PR file, R6b once a worktree subsection is in play), and the backfill of every existing entry into per-entry files — and half of it is worse than neither. R3 alone routes entries into `docs/changelog/` while R1 still requires reading a file nothing writes, so new entries are invisible at session start; the landing sequence left alone still folds a fragment into a file nothing appends to; and the layout without the backfill drops the history from required reading and puts an empty folder where it was. Offer it as a single item, priced (see R3's migration in the checklist — the backfill is the expensive step and gets delegated), and if the user tries to split it, say why and take all or none.

**R1's digest conversion is a separate, much smaller item** and can be offered on its own: a repo carrying a `docs/CHANGELOG-DIGEST.md` from an earlier version of this skill has its lines folded back into the entries they name, then the file deleted (R1's migration in the checklist). It runs inline, needs no fan-out, and is independent of which changelog layout the repo is on — but it is still a conversion, so lines that resolve to no entry get reported rather than dropped.

**It is internally all-or-nothing, like the layout migration.** Separable from everything else on offer, not separable from itself: taking the fold without the delete leaves a file that is now duplicated into the entries and still appended to, and — **when the file exists**, which is the combination this applies to; the checklist's four combinations govern the rest — taking the re-point or the R3 strip without the fold orphans the digest — a file every milestone still reaches for and no session ever loads, which is the worst of the four states. If the user wants to split it, say that and take all or none.
...
### 7. Create nothing for the changelog or the follow-up list

Counter-intuitive enough to be a step rather than an omission: a fresh scaffold creates **no** changelog file of any kind, and no follow-up list either.

- **Don't create `docs/changelog/`.** The first entry creates it. Git can't track an empty directory, and a `.gitkeep` in a folder that will be permanently populated is dead weight nobody ever deletes. (This is a *fresh scaffold* rule. The step-6 migration does create the folder — it has hundreds of backfilled entries to put in it.)
- **Don't create a digest, decision log, or any other curated summary** — not `docs/CHANGELOG-DIGEST.md`, not under some other name, not even empty. Distant history is reached by the full `ls docs/changelog` index plus a grep over the folder, both of which the root CLAUDE.md mandates as numbered actions (step 4 wrote them). A curated file is one that every change appends to and every compaction rewrites, which is exactly the collision the per-entry folder exists to remove; the append conflicts trivially, but the rewrite is a real semantic conflict and it falls due precisely when there is enough parallel work to make it expensive.
- **Don't create `docs/CHANGELOG.md`.** The convention is the folder.
- **Don't create `docs/CLAUDE_TODO.md`.** The first owed follow-up creates it, and a scaffold run has none — nothing has been deferred yet. An empty one is worse than absent for a reason specific to this file: it is agent-owned, and a committed empty TODO in a fresh repo reads as an invitation for the user to fill it, which inverts the ownership the whole convention rests on. Absence is also this file's *healthy* state rather than a not-started one, which is why step 4's template reads it as an action and treats missing as "nothing outstanding".

There is no dangling-reference problem to design around, because none of this is `@`-referenced: the changelog read is an *action*, and `ls` against a missing folder is a non-event the template already tells the session to expect ("an absent folder just means no entries yet"). The `docs/CLAUDE_TODO.md` read has the same shape for the same reason — an action, with a missing file meaning nothing is outstanding rather than something unfinished.

**If the repo already carries a `docs/CHANGELOG-DIGEST.md`, say so before finishing — even on a pure scaffold run.** Normally this is an **audit** finding: R1's migration in `references/audit-checklist.md` folds its lines back into the entries they name and then deletes it, and only if the user picks it. What this step must never do is delete it as a side effect of "create nothing for the changelog" — the file is somebody's record, and removing it is a conversion the user has to pick.

But a repo can hold a digest and *no* root `CLAUDE.md` — someone deleted the file, or kept the docs and dropped the scaffolding — and that repo goes down the scaffold path, where the audit never runs. The `CLAUDE.md` this step just wrote has no digest reference, so finishing quietly would orphan the file: still appended to by habit, never loaded by anything. So run the same `test -f docs/CHANGELOG-DIGEST.md` — unless step 6 ran at all, in which case it already made this offer and step 7 stays silent — and if it hits, offer R1's migration here too — steps 1–4 apply unchanged, and step 5's R1 half is already satisfied by the fresh file. If the user declines, name the orphan explicitly in step 10's summary rather than letting it pass as a clean scaffold.
...
- Reminders:
  ...
  - **The changelog read is four actions and no `@`-reference at all** — nothing can `@`-reference "the files of a folder". The session lists the folder, reads the newest 10, delegates a digest of what's behind them to a cheap subagent when there's enough there to be worth it, and greps before touching an area. Any of them can be skipped, and it fails quietly when they are — the grep especially, since nothing signals a decision you never thought to look for. That is the accepted cost of having no curated digest: such a file is one every change appends to and every compaction rewrites, which is the collision the per-entry folder exists to remove. The delegated digest is how the idea survives without the file — generated per session and never persisted, so nothing in git contends for it and it cannot go stale, since staleness needs persistence. Its cost is nondeterminism, which is why the template fixes the *class* to surface rather than asking for a summary: the prompt is the durable artifact. If you want the read deterministic, a session-start hook is the way (`update-config`) — out of this skill's scope, so yours to add.
  - **`docs/CLAUDE_TODO.md` is a list the agent writes and drains — not one it works from.** The root CLAUDE.md carries an *Owed follow-ups* section: work a session found and deliberately didn't do gets one bullet, the same discovery also goes into that change's changelog entry (the entry keeps the reasoning, the list keeps the present tense), and the bullet is deleted by the change that completes it. Nothing was created for it — the first deferral creates the file, and its absence means nothing is outstanding. The rule worth knowing: **an item on the list is never a reason to start work.** It gets surfaced when you ask for a change in that area, and you decide whether it comes into scope — and four things you can say drive it: **"what's outstanding?"** reads it back, **"do that one"** works an item and deletes it, **"drop it"** deletes without doing (the one deletion that isn't a completion), and **"add X to the TODO"** records one. The file itself carries a fixed note explaining all this to anyone who opens it in the repo, and beyond that has no structure at all — no grouping, priorities, dates or done section, each left out for a reason the section states. Lines you type in yourself are read as requests and asked about, not adopted — if you'd rather it were a proper inbox you fill, that's a deliberate change to the section, at the cost of a list that no longer reliably drains.
...
- Never create `docs/CLAUDE_TODO.md` on a scaffold run (step 7), and never adopt the contents of a pre-existing one as agent-written (R8's disk check) — those items were somebody's requests, so list them back and ask. Deleting an item is reserved for the change that completes it.
- Never modify a landed file in `docs/changelog/`. Its path is its address; renaming or deleting one breaks every citation to it and produces no conflict to catch. R1's migration makes the single exception: folding a digest line into an entry's `## Detail` adds to the record at its existing address, and never moves or rewrites it.
- Never delete an existing `docs/CHANGELOG-DIGEST.md` outside R1's migration, and never inside it while any line still resolves to no entry. The digest is redundant *by construction* — every line names an entry that already exists — but "by construction" is a claim to verify per line, not to assume.

## Scope

**In scope:** writing CLAUDE.md scaffolding, establishing the agent-owned `docs/CLAUDE_TODO.md` follow-up convention in the root file (step 4's template section, R8 in the audit rubric — the file itself is never created here, per step 7), auditing and surgically updating existing CLAUDE.md files against the current recommendations (step 6, rubric in `references/audit-checklist.md`), converting away a `docs/CHANGELOG-DIGEST.md` left by an earlier version of this skill (step 6, R1's migration — fold each line into the entry it names, report what doesn't resolve, then delete the file), migrating a repo from the retired single-file changelog to the per-entry `docs/changelog/` folder when the audit finds evidence of pressure (step 6 — one all-or-nothing item that splits every existing entry into a dated file, verifies the split, and removes the original changelog files from the working tree; delegated as a fan-out, and only if the user picks it), optionally wiring git-describe app versioning (step 8, only if the user opts in — per-ecosystem recipes live in `references/app-versioning.md`), checking for the `fix-code` and `git` companion skills and offering to install the missing ones user-level (step 9, opt-in), and — for UI subtrees only — verifying Playwright MCP is installed and offering to install it headless if it isn't (`references/playwright-mcp.md`).
