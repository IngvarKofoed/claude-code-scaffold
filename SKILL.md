---
name: scaffold
description: "Bootstrap a repository's CLAUDE.md scaffolding once `docs/CONCEPT.md` and `docs/ARCHITECTURE.md` already exist. Use this skill when the user says any of: 'scaffold this repo', 'set up CLAUDE.md', 'bootstrap this repo for Claude', 'initialize the agent scaffolding', 'create the CLAUDE.md files', 'wire up the CLAUDE.md', or any phrasing that implies a fresh repo needs its CLAUDE.md files written now that the anchoring docs are done. Also triggers on 'I've written my CONCEPT and ARCHITECTURE, now what?' or 'create root and subtree CLAUDE.md'. Writes root + per-subtree CLAUDE.md from templates in this skill's folder, establishes the per-entry `docs/changelog/` convention, optionally wires git-describe app versioning, prints next steps. When CLAUDE.md files already exist it never overwrites blind: it audits each one against this skill's current recommendations, reports per file what's missing or outdated (e.g. an old review mandate, a changelog section still on the retired single-file numbered scheme, a required-reading block still loading a curated changelog digest), and asks which files to bring up to date — applying only surgical, additive updates that preserve project-specific content. So also trigger this skill on 'audit my CLAUDE.md', 'are my CLAUDE.md files up to date', 'check my CLAUDE.md against the latest recommendations / scaffold conventions', or 'update CLAUDE.md to the current conventions'."
---
...
One rubric item can't be judged from the files at all: **`test -f docs/CHANGELOG-DIGEST.md`**. R1's digest finding is keyed to the file on disk, not to whether a `CLAUDE.md` references it — a digest whose reference was hand-removed is still a file people append to and nothing reads, and reading only the `CLAUDE.md` files would never see it. Run the check once, here, and pair it with what the text says (R1's migration in the checklist enumerates the four combinations).
...
### 7. Create nothing for the changelog

Counter-intuitive enough to be a step rather than an omission: a fresh scaffold creates **no** changelog file of any kind.
...
There is no dangling-reference problem to design around, because none of this is `@`-referenced: the changelog read is an *action*, and `ls` against a missing folder is a non-event the template already tells the session to expect ("an absent folder just means no entries yet").
