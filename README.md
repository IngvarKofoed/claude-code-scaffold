...
A [Claude Code](https://docs.claude.com/en/docs/claude-code) skill that bootstraps a repository's `CLAUDE.md` scaffolding once `docs/CONCEPT.md` and `docs/ARCHITECTURE.md` exist. Writes a root `CLAUDE.md` and a scoped `CLAUDE.md` per major subtree, wiring in the per-entry `docs/changelog/` convention — and creates no changelog file itself, since the first entry creates the folder and there is deliberately no summary file beside it. It never overwrites an existing `CLAUDE.md` blind: when a file already exists, it **audits** it against the skill's current recommendations, reports per file what's missing or outdated, and asks which files to update — applying surgical, additive edits that preserve your project-specific content.

The opinion behind it: anchoring docs + tight imperative `CLAUDE.md` files + a changelog of one file per change (`docs/changelog/YYYY-MM-DD-<slug>.md`, read as a full filename index plus the newest few entries and a grep before you touch anything, so no two parallel changes can collide anywhere — there is no shared summary file either — and session start doesn't pay for the whole history) + a single review point at the commit — one review-grade pass over the accumulated diff, offered when you ask to commit and run by the agent itself (a deep `/code-review high --fix` or the tighter `/fix-code --fix`, your pick, inline when that skill isn't installed) rather than a pass after every edit — is the minimal scaffolding that makes a repo productive to work in with Claude. Domain skills, memory, and hooks are out of scope — add those separately when the project actually needs them.
