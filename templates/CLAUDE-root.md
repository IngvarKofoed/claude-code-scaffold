...
- **The file has two tiers, and only the first is bounded.** A `#` heading, then a **lead** of **1–5 lines, ~20 words per line at most** — never one packed run-on line, since a 40-word run-on isn't a short entry, it just hides the bulk on a single line. Below it, an optional **`## Detail`** section for what a future session must know and cannot get from the diff: the alternative you rejected (so nobody re-introduces it), a constraint that still binds, a known limit or deferred follow-up, what your verification did and didn't cover. The lead is bounded because it is what a session reads to orient; the detail is bounded by *purpose*, because nothing else bounds it now that entries no longer share a file. Past ~40 lines of detail the change wanted a spec — write one and link it.
