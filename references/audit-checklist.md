...
After a non-trivial edit, the change **gets a review-grade review automatically**, with a two-path shape: **`/fix-code --fix` when that skill is installed**, and otherwise the agent's own inline pass — scaled to the change: a careful single-pass read of the diff for small edits, and for substantial or complex ones a **fan-out across subagents (the Agent tool — no ultracode opt-in needed) one per at-risk dimension (~2–4), each finding verified before it's acted on** (finders on the strong model, verification a tier down — see R7). It checks correctness, security & data-integrity, edge cases & tests, reuse, clarity, performance, and conformance to the repo's own conventions; applies **every** fix automatically; and reports grouped by severity (blocker / should-fix / nit), one line per bucket, without stopping to ask which to fix. Plus a plain statement of anything it couldn't fully verify — a guessed intent, a known gap, an uncovered path — so the user knows where their own judgement is still needed. Plus one **pre-commit gate**: when the user asks to commit and the working tree holds non-trivial changes no `/fix-code --fix` run has seen, the agent asks first (`AskUserQuestion`) whether to run `/fix-code --fix` over the working diff before committing or to commit as-is — the agent runs the pass either way. Gated on `fix-code` being installed, skipped for trivial diffs or when `/fix-code --fix` already covered this working diff, asked once per commit request, and never a substitute for the round's own review.
- *Why:* review-quality assurance on every change **without anyone having to invoke it** — the point is walk-away autonomy, so the agent runs the review itself, fanning out via the Agent tool for big changes. A mandate that defers the review to some tool the agent can't reliably reach just fails and emits an apologetic note every edit.
- *The self-review's rigor is the project's call* — different line counts or a different risk-surface list are both fine. Only the structural drifts below are findings.
- *Outdated looks like* (this is the most common drift, and the highest-value find):
  - **makes an external review command the *only* path** — named with no inline fallback, so the mandate goes dead in any checkout where that skill isn't installed and the agent emits an apologetic note instead of a review. A **conditional** preference is correct and must **not** be flagged: "`/fix-code --fix` when installed, otherwise review inline" is the current convention, and a file carrying only the inline pass is Outdated in the mild sense — offer the preference, don't rewrite the fallback;
  - describes the self-review as a **cursory single glance** with no subagent fan-out / finding-verification even on substantial or complex changes — the intent is review quality run automatically (fan-out via the Agent tool for big changes), not a token read;
  - tells the agent to **ask the user which findings to fix** instead of auto-fixing all of them;
  - **routes the user to a separate review pass** — an end-of-turn nudge to run some review command, gated on soft adjectives or hardcoded to one level. Fired on nearly every turn it is pure noise the user learns to skip, and it implies the self-review above was provisional. Current convention: no end-of-turn nudge — the review already happened, so just state what couldn't be verified and stop. The **pre-commit gate** described above is the one sanctioned stop-and-ask and must **not** be flagged as this drift: it fires only on an explicit commit request (discrete and infrequent, not per turn), the agent runs the pass itself rather than routing the user elsewhere, and it's skipped when a full pass already covered the diff. A file without that gate is Outdated only in the mild sense — offer it, don't manufacture a finding.
