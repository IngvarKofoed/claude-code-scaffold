...
- *Outdated looks like:* R7 is present but covers only *authoring* (setting `model` per `agent()` call) and omits the **named/invoked-workflow** case — that invoking a pre-built workflow runs all its stages on the session model unless its `scriptPath` is edited to tier the checking stages down, with `code-review`'s verifiers as the concrete default. Flag the missing named-workflow guidance as Outdated (only where R7 is otherwise warranted).
