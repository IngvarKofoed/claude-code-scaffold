...
After a non-trivial edit, the agent **runs a code-review-grade review itself, automatically** — scaled to the change: a careful single-pass read of the diff for small edits, and for substantial or complex ones a **fan-out across subagents (the Agent tool — no ultracode opt-in needed) one per at-risk dimension (~2–4), each finding verified before it's acted on** (finders on the strong model, verification a tier down — see R7). It checks correctness, security & data-integrity, edge cases & tests, reuse, clarity, performance, and conformance to the repo's own conventions; applies **every** fix automatically; and reports grouped by severity (blocker / should-fix / nit), one line per bucket, without stopping to ask which to fix. Plus an end-of-turn nudge to run a user-initiated `/code-review`, which **defaults to silence** and fires only on a **named trigger** (heavy self-review / high-consequence surface / large non-mechanical diff / multi-agent-produced diff / genuine uncertainty), at a **chosen level** (`medium` ordinarily; `high` for a high-consequence surface, more than one blocker fixed, or two-plus triggers) — framed as optional reassurance on a fresh, independent pass, not a warning that something is wrong.
- *Why:* code-review-quality assurance on every change **without anyone having to invoke it** — the point is walk-away autonomy, so the agent runs the review itself, fanning out via the Agent tool for big changes (the `/code-review` skill is user-invoke-only via `disable-model-invocation` and the Workflow tool needs per-turn opt-in, so neither can be the automatic path — a mandate to "invoke `code-review`" instead just fails and emits an apologetic note every edit). The nudge is the **exception path**: an independent second opinion offered only when it adds confidence. Fired on every diff it becomes noise the user learns to skip.
- *The self-review's rigor, and the nudge's exact thresholds, are the project's call* — different line counts, a different risk-surface list, or a different level ladder are all fine. Only the structural drifts below are findings.
- *Outdated looks like* (this is the most common drift, and the highest-value find):
  ...
  - has the routine review mandate but **no end-of-turn nudge to suggest a deliberate user-run `/code-review` pass at all** — so the heaviest, riskiest changes ship without ever offering the user an independent second pass. (Flag its absence as Outdated. Conversely, do *not* flag a nudge for firing narrowly — narrow is correct.)
  - has the nudge, but gated on **soft adjectives** ("when the change is significant / complex / churned a lot") with **no default of silence**, and/or with a **single hardcoded level** (`/code-review medium`). This is the highest-frequency drift and it's invisible in the file — it only shows up in behavior: the agent suggests a pass on nearly every turn, always at the same level, so the signal carries no information. Current convention: say nothing unless a **named** trigger fired (heavy self-review — a real blocker fixed or should-fixes across most touched files; high-consequence surface — authn/authz, secrets, money, migrations/data deletion, concurrency, published contracts; large *and* non-mechanical; multi-agent-produced diff; genuine uncertainty), an explicit **not-a-trigger** list (docs-only, formatting, dep bumps, added tests, contained single-function changes, nits-only reviews), and a **level rule** rather than one hardcoded level. Also flag it if the nudge is present and triggered but never names *which* trigger fired.
...
- *Outdated looks like:* R7 is present but (a) covers only *authoring* (setting `model` per `agent()` call) and omits the **named/invoked-workflow** case — that invoking a pre-built workflow runs all its stages on the session model unless its `scriptPath` is edited to tier the checking stages down, with `code-review`'s verifiers as the concrete default; or (b) omits the **self-review-on-return** tie-back — that when a workflow lands edits, the R5 self-review applies to the aggregate diff and fires R5's multi-agent nudge trigger. Flag either missing piece as Outdated (only where R7 is otherwise warranted).
